...
public abstract class Scene {
    ...
    public void run(final SceneBuilder builder, final SceneBuildingUtil util) {
        ...
        final VectorUtil vectorUtil = util.vector();
        ...
        scene(scene, world, overlays, select, vectorUtil, effects, grid);
    }

    public abstract void scene(final CreateSceneBuilder scene, final CreateSceneBuilder.WorldInstructions world,
                               final OverlayInstructions overlay, final SelectionUtil select, final VectorUtil vectorUtil,
                               final EffectInstructions effects, final PositionUtil grid);


    protected static AABB getDiodeBaseOutline(BlockPos analogGatePos) {
        return new AABB(analogGatePos).inflate(-0.05f, -0.45f, -0.08f).move(0, -.45f, 0);
    }
