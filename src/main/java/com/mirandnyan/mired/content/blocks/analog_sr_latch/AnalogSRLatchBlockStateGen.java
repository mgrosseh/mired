...
public class AnalogSRLatchBlockStateGen {
    ...
    public static <P extends AnalogSRLatchBlock> NonNullBiConsumer<DataGenContext<Block, P>, RegistrateBlockstateProvider> generate() {
        return (ctx, prov) -> {
            ...
            for (final BlockState state : ctx.get().getStateDefinition().getPossibleStates()) {
                final Direction facing = state.getValue(AnalogSRLatchBlock.FACING);
                final boolean powered = state.getValue(AnalogSRLatchBlock.POWERED);
                final boolean sidePowered = state.getValue(AnalogSRLatchBlock.SIDE_POWERED);
                final boolean powering = state.getValue(AnalogSRLatchBlock.POWERING);
                ...
                builder.part()
                        ...
                        .rotationY(yRot)
                        .addModel()
                        .condition(AnalogSRLatchBlock.FACING, facing)
                        .end().part()

                        .modelFile(powered ? backOn : backOff)
                        .rotationY(yRot)
                        .addModel()
                        .condition(AnalogSRLatchBlock.POWERED, powered)
                        .condition(AnalogSRLatchBlock.FACING, facing)
                        .end().part()
                        ...
                        .rotationY(yRot)
                        .addModel()
                        .condition(AnalogSRLatchBlock.SIDE_POWERED, sidePowered)
                        .condition(AnalogSRLatchBlock.FACING, facing)
                        .end().part()
                        ...
                        .rotationY(yRot)
                        .addModel()
                        .condition(AnalogSRLatchBlock.POWERING, powering)
                        .condition(AnalogSRLatchBlock.FACING, facing)
                        .end();
            }
        };
    ...
    private static ModelFile sub(final RegistrateBlockstateProvider p, final String suffix) {
        return p.models().getExistingFile(Mired.path("block/analog_sr_latch/" + suffix));
    }
}
